/**
 * Web slot bus(L5-B4.2 / W4.2 C3 迁入 capability)
 *
 ...
 *
 * 用途:
 * - SyncDriver 跨 slot 发送 SYNC_EVENTS / NAVIGATE / TAKE_CONTROL(driver 通过
 *   bus 接口注入访问,不直接 import 本模块)
 * - capability 内 Host / TranslateHost 互发 REQUEST_URL / NAVIGATE 等控制消息
 *
 * 历史(W4.2 C3):文件原本位于 src/views/web/slot-bus.ts。slot-bus 不进 shared
 * (audit review P2-B 决议)— 'left'/'right' 是业务语义 + 有状态实例,
 * 违反 shared 中性原则。capability 内部消化最干净。
 *
 * 边界:不在 capability index.ts re-export(对外不可见),只供 capability 内
 * 模块通过相对路径 import。
 */

import type { Side, SlotMessage } from '@drivers/web-sync-driver';
