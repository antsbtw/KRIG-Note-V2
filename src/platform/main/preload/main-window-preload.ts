...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  /** 主动查询当前系统主题(权威值，来自主进程 nativeTheme.shouldUseDarkColors) */
  getNativeTheme(): Promise<{ dark: boolean }> {
    return ipcRenderer.invoke(IPC_CHANNELS.NATIVE_THEME_GET);
  },
  ...
  /** L5-G7b:按 family 名读字体二进制(记名方案;没装该字体 → null,渲染层回退打包)。bold=要粗体变体 */
  async fontReadByName(family: string, bold?: boolean): Promise<ArrayBuffer | null> {
    return ipcRenderer.invoke(IPC_CHANNELS.FONT_READ_BY_NAME, family, bold === true);
  },
  ...
  xTimeline: {
    ...
    collectReplies: (handle: string, wcId?: number, targetDays?: number, mode?: 'incremental' | 'backfill') =>
      ipcRenderer.invoke(IPC_CHANNELS.X_COLLECT_REPLIES, { handle, wcId, targetDays, mode }),
