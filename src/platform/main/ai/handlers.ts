...
import { resolveAIWebContents } from './webview-registry';
...
export function registerAIHandlers(): void {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      const p = payload as { serviceId?: unknown; prompt?: unknown; options?: unknown; targetWcId?: unknown } | null;
      ...
      return askAI(p.serviceId, p.prompt, targetWcId, opts.timeoutMs);
    },
  );
  ...
  ipcMain.handle(IPC_CHANNELS.AI_OPEN_SESSION, async (_e, payload: unknown) => {
    ...
    const targetWcId = typeof p.targetWcId === 'number' ? p.targetWcId : undefined;
    // 按 ws 定向探测本 ws 的 AI 实例(治多实例串扰);未命中 fail loud 报具体原因。
    const got = resolveAIWebContents(p.serviceId, targetWcId);
    if ('error' in got) {
      return {
        success: false,
        status: 'not-attached',
  ...
  ipcMain.handle(IPC_CHANNELS.AI_PASTE_AND_SEND, async (_e, payload: unknown) => {
    const p = payload as { serviceId?: unknown; prompt?: unknown; targetWcId?: unknown } | null;
    if (!p || !isServiceId(p.serviceId) || typeof p.prompt !== 'string' || !p.prompt) {
      return { success: false, error: 'invalid pasteAndSend payload' };
    }
    const targetWcId = typeof p.targetWcId === 'number' ? p.targetWcId : undefined;
    return pasteAndSend(p.serviceId, p.prompt, targetWcId);
  });
  ...
  ipcMain.handle(IPC_CHANNELS.AI_EXTRACT_FULL, async (_e, payload: unknown) => {
    const p = payload as { serviceId?: unknown; targetWcId?: unknown } | null;
    if (!p || !isServiceId(p.serviceId)) {
      return { success: false, error: 'invalid serviceId' };
    }
    const targetWcId = typeof p.targetWcId === 'number' ? p.targetWcId : undefined;
    return extractFullConversation(p.serviceId, targetWcId);
  });
  ...
  ipcMain.handle(IPC_CHANNELS.AI_EXTRACT_TURN, async (_e, payload: unknown) => {
    ...
    const targetWcId = typeof p.targetWcId === 'number' ? p.targetWcId : undefined;
    return extractConversationTurn(p.serviceId, p.x, p.y, targetWcId);
  });
