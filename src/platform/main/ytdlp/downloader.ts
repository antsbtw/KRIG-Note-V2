...
async function exportWebviewCookiesForYoutube(): Promise<string | null> {
  try {
    ...
    if (cookies.length === 0) {
      console.log('[ytdlp cookies] webview 无 youtube/google cookies,需用户先在 web view 登录 YouTube');
      return null;
    }
    console.log('[ytdlp cookies] all cookie names:', cookies.map((c) => c.name).join(', '));
    ...
    console.log('[ytdlp cookies] exported', cookies.length, 'cookies →', cookiesFile);
    return cookiesFile;
  } catch (e) {
