 ...
 *
 * ⚠️ **判读顺序**(2026-09-02 踩过的倒果为因,别再犯):
 *   曾拿「库里 856 行 in_reply_to 全为 NONE」去论证「选择器取错了」——**不成立**。
 *   那 856 行全部来自关键词配方(from_accounts 为空,搜 VPN 求助),
 *   返回的本就是原创推,**样本里根本没有回复**,字段为空是应然不是异常。
 *   同理:`in_reply_to` 与「我们自己的回复功能」无关 —— 用户没用过那个功能,
 *   它空着也不说明任何问题。追踪的对象是 **X 上「他回复别人」那条推本身**,
 *   由 from:<handle> 搜索抓回,回复关系读自推文 DOM。
 *
 *   所以必须**先**用 from: 抓到确实含回复的一批,**再**看 in_reply_to 有没有值。
 *   那时候的空值才是真信号。顺序反了就是拿不含该现象的样本论证现象缺失
 *   —— 仪器给出读数前,先确认它测的是不是你以为的那个量(交接文档 §7.1)。
 *
