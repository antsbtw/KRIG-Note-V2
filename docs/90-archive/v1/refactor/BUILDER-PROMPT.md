...
> **使用说明**：每个重构阶段的全部 Builder 输入物**集中在一个目录** `docs/90-archive/refactor-v1/stages/<阶段编号>-<阶段名>/`，启动 Builder 时只需告知该目录路径——目录内包含 README、task-card、BUILDER-INSTRUCTION 等自包含文件。
>
...
3. **本次的阶段目录**：`docs/90-archive/refactor-v1/stages/<阶段>/`（路径由 Commander 在启动消息中给出）
