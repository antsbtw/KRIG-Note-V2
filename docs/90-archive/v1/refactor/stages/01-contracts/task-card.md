...
## 引用
- 总纲：[docs/90-archive/refactor-v1/00-总纲.md](../../00-总纲.md) v2.3 § 9 启动清单
...
段落末尾追加："违反以上任一条 = PR 拒绝合入。详见 [docs/90-archive/refactor-v1/00-总纲.md](docs/90-archive/refactor-v1/00-总纲.md)"
  ...
  rules: {
    'no-restricted-imports': ['error', {
      patterns: [{
        ...
        message: 'L5 插件禁止直接调布局特权 API。改用 dispatch(IntentEvent) — 见 docs/90-archive/refactor-v1/00-总纲.md § 1.1 分层原则',
      }],
    }],
  },
},
...
if [[ -n "$VIOLATIONS" ]]; then
  ...
  echo "  详见 docs/90-archive/refactor-v1/00-总纲.md § 5.8"
  exit 1
fi
