...
>
> **使用方式**:在新会话开头说"读 docs/90-archive/refactor-v1/pdf-viewer-adapter-prompt.md 执行",会话会自包含。
...
写一个 `docs/90-archive/refactor-v1/pdf-viewer-adapter-test-checklist.md` 给用户跑,含:
- 加载 / 翻页 / 滚动
- trackpad pinch 缩放体验
- Cmd+= / Cmd+- / Cmd+0
- outline 跳转
...
Stage 1 开始前,写 `docs/90-archive/refactor-v1/pdf-viewer-adapter-plan.md`,内容:
- 重构动机(一段)
...
3. 创建 `docs/90-archive/refactor-v1/pdf-viewer-adapter-plan.md`,把上面 5 个 stage 的目标 + adapter API 签名草稿落下来
