...
> 把以下完整内容复制到新对话即可启动实施。
>
> 任务自包含,不依赖任何对话上下文 — 所有设计依据都在 `docs/90-archive/tasks/` 下的 4 份文档里。
...
```
请按 docs/90-archive/tasks/dual-channel-implementation.md(阶段 1)实施 NoteView 光标
...
1. **docs/90-archive/tasks/cursor-jump-rootcause.md** — 问题分析
   - 8 段链路梳理(用户按键 → IPC → broadcast → useEffect[doc] → replaceWith)
   - 3 个并发设计成因
   - 已打补丁定性(全部症状级)

2. **docs/90-archive/tasks/noteview-sync-architecture-decision.md** — 方案对比
   - 8 个候选方案矩阵
   - 选定 #4(双 channel)

3. **docs/90-archive/tasks/dual-channel-implementation.md** — **实施清单(你要执行的)**
...
4. **docs/90-archive/tasks/host-ref-based-checklist.md** — **不在本 PR 范围**,只作未来参考
...
2. 完整读 docs/90-archive/tasks/dual-channel-implementation.md(尤其 §2 文件改动 + §5.1)。
...
把上面三个反引号包裹的部分(从"请按 docs/90-archive/tasks/dual-channel-implementation.md"
开始,到"等用户说 ... 之后再动手。"结束)整段复制到新对话窗口的第一条消息发送
即可。

新对话拿到提示词后会:
1. 读 4 份文档(全在仓库 docs/90-archive/tasks/ 下)
...
*生成于 2026-05-20,配套 docs/90-archive/tasks/dual-channel-implementation.md 阶段 1 实施*
