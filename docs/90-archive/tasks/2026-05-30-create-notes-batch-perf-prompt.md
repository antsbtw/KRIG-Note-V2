...
`docs/90-archive/refactor-v2/data-model/persistence/decisions/020-sub-phase-3a-tx-true-atomicity.md` §4.1 / §5.2 字面拍板 ViaTx helper 走 `tx.query` 替代 `db.query`,本身不涉及"是否批量".本 prompt 不破 decision 020.
...
docs/90-archive/refactor-v2/data-model/persistence/spec.md §6 PE4 字面拍板"createNotesBatch 单事务 + 三类边(belongsToNote / childOf / nextSibling)" — 本 prompt 不破这条 spec.单事务保留,仅优化事务内 round-trip.
