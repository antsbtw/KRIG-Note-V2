...
你是 KRIG-Note V2 的**实施工程师**。本次任务**重做** Stage 7 — 上一轮 subagent 的 Stage 7 已被 `git reset` 丢弃，因为它（连同 Stage 5/6 的部分思路）把 V1 遗留的 `AtomInput` 概念提升到 semantic 层冒充 SSOT，**反 V2 数据模型规范**（[`docs/90-archive/refactor-v2/data-model/README.md`](../RefactorV2/data-model/README.md) §"当前 V2 状态" 字面登记 AtomInput 为"未对齐统一 Atom 定义"的 V1 遗留）。
...
[`docs/90-archive/refactor-v2/data-model/`](../RefactorV2/data-model/) 是 V2 数据模型权威规范。本期实施必须字面对齐以下规范点：
