...
文件：[docs/90-archive/refactor-v2/data-model/persistence/decisions/026-block-atomization-sub-phase-design.md](docs/90-archive/refactor-v2/data-model/persistence/decisions/026-block-atomization-sub-phase-design.md)
...
- [decision 026 §3.1.4](docs/90-archive/refactor-v2/data-model/persistence/decisions/026-block-atomization-sub-phase-design.md) 写"1 table atom（根容器，不拆）→ table.content = []"。这一行**暗示 table 是 atom**——与 §3.1.2 表头"table | 表格根容器 | 不拆"矛盾。
- [decision 026 §3.4 例 3](docs/90-archive/refactor-v2/data-model/persistence/decisions/026-block-atomization-sub-phase-design.md) PM payload 字面写：
  ```ts
  { domain:'pm', payload:{ type:'table', attrs:{id:'<ULID>'}, content:[] } }
  ```
  ——再次暗示 table 有 atom + 有 attrs.id。
- [decision 026 §6.1](docs/90-archive/refactor-v2/data-model/persistence/decisions/026-block-atomization-sub-phase-design.md) 字面写："tableCell.childOf → table atom（跳过 tableRow）"——**要求 table 必须是 atom**，否则 childOf 无目标。
...
- 决议字面：[decision 026 §13.1-13.8](docs/90-archive/refactor-v2/data-model/persistence/decisions/026-block-atomization-sub-phase-design.md#L810) **没有"tableCell 跨 row 拼装信息丢失" 条目**。最接近的 §13.7 只问 tableHeader 是否拆 atom。
