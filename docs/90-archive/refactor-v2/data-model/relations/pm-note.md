...
| 节点 type | 子文档 | 状态 |
|---|---|---|
| `image` | [blocks/image.md](pm-note/blocks/image.md) | ✓ 批 3 |
| `fileBlock` | [blocks/file-block.md](pm-note/blocks/file-block.md) | ✓ 批 3 |
| `audioBlock` | [blocks/audio-block.md](pm-note/blocks/audio-block.md) | ✓ 批 3 |
| `videoBlock` | [blocks/video-block.md](pm-note/blocks/video-block.md) | ✓ 批 3 |
| `tweetBlock` | [blocks/tweet-block.md](pm-note/blocks/tweet-block.md) | ✓ 批 3 |
| `externalRef` | [blocks/external-ref.md](pm-note/blocks/external-ref.md) | ✓ 批 3 |
...
| 节点 type | 子文档 | 状态 |
|---|---|---|
| `table` | [blocks/table.md](pm-note/blocks/table.md) | ✓ 批 3 |
| `tableRow` | [blocks/table-row.md](pm-note/blocks/table-row.md) | ✓ 批 3 |
| `tableCell` | [blocks/table-cell.md](pm-note/blocks/table-cell.md) | ✓ 批 3 |
| `tableHeader` | [blocks/table-header.md](pm-note/blocks/table-header.md) | ✓ 批 3 |
...
| `fileLink` | 文件链接 atom | [inlines/file-link.md](pm-note/inlines/file-link.md) | ✓ 批 3 |
...
| `strike` | 删除线 | 同上 | ✓ 批 1 |
| `code` | 行内代码 | 同上 | ✓ 批 1 |
| `textStyle` | 文字样式 mark（携带 color） | [marks/text-style.md](pm-note/marks/text-style.md) | ✓ 批 3 |
| `highlight` | 高亮（携带 color） | [marks/highlight.md](pm-note/marks/highlight.md) | ✓ 批 3 |
| `link` | 超链接 | [marks/link.md](pm-note/marks/link.md) | ✓ 批 3 |
...
### 3.1 字段优先级规则（强制约定，编写纪律）

**framework-injected 字段在 node-declared 中不重复声明**：同名字段若已由 schema 框架注入（如 `indent`），节点级 attrs **不重复声明**，避免 schema 重复定义错误。

适用范围：所有 block 子文档（`pm-note/blocks/*.md`）写 §3 attrs schema 时，按此规则区分"框架级"与"节点级"两段。

⚠ **本规则是 schema 编写时的"避免重复声明"约束**。运行时同名字段冲突的解析（实体元属性 / atom 数据 / 框架级注入三层命名空间隔离）由 [`persistence/spec.md §4.2`](../persistence/spec.md) 定义，两份规则作用域不同、互不冲突。
...
媒体 block（6 份）：
- ✓ `blocks/image.md`
- ✓ `blocks/file-block.md`
- ✓ `blocks/audio-block.md`
- ✓ `blocks/video-block.md`
- ✓ `blocks/tweet-block.md`
- ✓ `blocks/external-ref.md`

表格 block（4 份）：
- ✓ `blocks/table.md`
- ✓ `blocks/table-row.md`
- ✓ `blocks/table-cell.md`
- ✓ `blocks/table-header.md`

inline / mark（4 份）：
- ✓ `inlines/file-link.md`
- ✓ `marks/text-style.md`
- ✓ `marks/highlight.md`
- ✓ `marks/link.md`
