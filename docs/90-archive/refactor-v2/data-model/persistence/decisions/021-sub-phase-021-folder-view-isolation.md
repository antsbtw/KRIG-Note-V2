...
| `docs/90-archive/refactor-v2/notes/L7-next-phase-kickoff.md` | §1.4 Q-shared-folder-ux + §1.5 设计师教训表(第 14 次 / 第 15 次) | 标"已解决 decision 021" + 加第 14 / 15 次教训 |
| `docs/90-archive/refactor-v2/data-model/persistence/SDK-version-binding-policy.md` | §2.2 第 6 / 7 步 + §6 修订记录 v1.3 | 第 6 步加"启动包 / 总纲文档字面区分现状 vs 未来计划"(第 14 次);第 7 步加"决议引用 storage API / 类型字段前必须 grep 真实接口字面 + LiteralValue 字面三字段 `{ kind, type, value }` 缺一不可"(第 15 次);§6 加 v1.3 修订记录指向 decision 021。**授权依据**:§0.5.bis 用户 2026-05-13 P0 显式授权 |
| `docs/90-archive/refactor-v2/data-model/relations/krig-vocab.md`(若存在) | folderForView 条目 | 新增登记 |
...
### 11.8 第 21 次设计师教训:完成报告字面与 git log 字面双向核对(Step 5.8 总指挥审计触发)

**本次教训**:实施者 Step 5.8 完成报告字面**首段总述**写"7 commit",但 grep `git log feature ^main` 实证**实际 6 commit**;同报告**table 字面**自校验"6 commit(实际 6 + Step 5.4 是 fix commit 同样在 commit step 范围内)"字面已纠正,但首段总述未同步纠正 — 字面**首段 vs table 不一致**。

**触发场景特性**:
- 完成报告字面跨多段(首段总述 + table 矩阵 + 完成判据 + 偏离汇总)— 字面**任一段修订时其他段字面未同步**
- 实施者自校验 table 字面字面准确,但**首段总述字面笔误未自校验**
- 字面失误本身无实质偏离(代码字面正确 + grep 实证一致),但**字面一致性纪律**字面同 decision 020 §11.4 第 11 次教训"决议字面 grep verify 双向核对"同型升级

**纪律升级**(跟第 11 / 15 次教训同型 — "双向核对"模式扩展):
- **完成报告字面"X commit"必须字面 git log verify**(沿 §0.2 第 1 条"严格 cd"同型字面纪律,本次扩展到"严格 git log verify")
- **完成报告字面跨段一致性自校验**:首段总述 / table / 完成判据 / 偏离汇总字面**字面互相核对**,任一段修订时其他段字面同步
- 字面纪律登记:**实施者纪律累积教训表追加第 6 条**(沿 [L7 启动包 §1.6](../../../notes/L7-next-phase-kickoff.md) 实施者纪律累积教训表)— "完成报告字面跨段一致性 + git log 字面 verify"

**项目级落地**(用户 2026-05-13 字面"测试通过了"后,沿 Step 5.7 §0.5.ter P0 授权字面纪律):
- **本决议范围内字面登记**:不再扩 SDK-policy 第 9 步(避免每次累积教训都扩 policy,SDK-policy §2.2 第 8 步字面已承载"字面纪律前瞻"完整 6 层 grep,第 21 次教训属"完成报告纪律"非"决议字面 grep 纪律")
- **L7 启动包 §1.6 实施者累积教训表追加第 6 条**(本次收尾 commit 一并字面落地)

**未来字面应用**:sub-phase 022 / 023 等后续 sub-phase 完成报告字面**必须**:
1. `git log feature ^main` 字面 verify commit 数
2. 完成报告字面跨段(首段 / table / 完成判据 / 偏离汇总)字面一致性自校验
3. 字面 commit 数 mismatch 立即字面纠正,不等总指挥审计发现
