...
→ [`docs/90-archive/refactor-v2/data-model/atom/spec.md`](../data-model/atom/spec.md) §0.1 字面:"Atom = V2 语义层的**最小**实体"。§2.2 字面:
...
- ❌ **未找到** decision 030(decision 022 字面提到的"decision 030+ 大架构升级")的实际文档。`docs/90-archive/refactor-v2/data-model/persistence/decisions/` 字面最高编号是 024
- ❌ **未找到** `docs/90-archive/refactor-v2/notes/L7-next-phase-kickoff.md` 列出的待启动 sub-phase 中包含"block 颗粒度升级"或"atom 形态重做"
...
**反证 / 不足**:
- ❌ V2 重构的决议系统(`docs/90-archive/refactor-v2/data-model/atom/decisions/` 和 `docs/90-archive/refactor-v2/data-model/persistence/decisions/`)字面**未引用上述 V1 时代决议作为延后依据**。当前 V2 实施层的"整篇 doc = 1 atom"是 [decision 012 §3.2](../data-model/persistence/decisions/012-sub-phase-2-note-folder-migration.md) 字面拍板,**没有"延后到未来 sub-phase"的对话** — 该决议直接采用整篇,未列替代方案
...
| # | 文档 | 关键收获 |
|---|---|---|
| 1 | [docs/90-archive/refactor-v2/data-model/README.md](../data-model/README.md) | Atom + 边设计哲学 + Phase 推进表 + sub-phase 进度 |
| 2 | [docs/90-archive/refactor-v2/data-model/atom/spec.md](../data-model/atom/spec.md) | "Atom = 最小实体" + "pm atom 最小单元 / block 是组合形态" 字面定义 |
