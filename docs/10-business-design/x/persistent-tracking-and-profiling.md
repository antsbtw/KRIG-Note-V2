...
#### (3.5) 存储隔离:X 走独立 database(已拍板)

**用户 2026-09-01 决定:取 B(独立 database),且允许推倒 X 现有表重建,
前提是保证严格的数据隔离。**
...
| 档 | 隔离度 | 判断 |
|---|---|---|
| A. 表名前缀 | 逻辑隔离;备份/清理/migration 仍与笔记绑死 | **不够** —— 达不到"严格隔离" |
| **B. 独立 database** | 独立 schema / migration / 备份 / 清理;跨库无法误 JOIN;删整库不波及笔记 | **足够** ✓ |
| C. 独立 SurrealDB 实例 | 进程级隔离 | 收益仅剩"进程崩溃互不影响",但代价是多一个 sidecar 要管(启停/健康检查/凭据/端口),**性价比不成立** |

B 已经满足严格隔离的实质要求:**数据在物理上分属不同 database,
SurrealDB 层面无法跨库 JOIN,备份和删除粒度独立**。C 多出来的那点隔离
换不回它的运维复杂度。

##### 必须做的基础设施改造

现状:`storage/surreal/client.ts` 是**单例模型** —— `getDB()` 返回唯一 Surreal 实例,
ns/db 在 `connect()` 时硬编码绑定([client.ts:20-21](../../../src/storage/surreal/client.ts#L20))。

改造要点:

1. **多连接支持** —— 按 database 名维护连接池:`getDB()` 保持现语义(笔记库),
   新增 `getXDB()`(或 `getDB('krig_x')`)返回 X 库连接
2. **重连恢复必须逐连接生效** —— 现有注释记录了踩过的坑:
   一次性 `signin()`/`use()` 在 SDK 自动重连后不重放 → 全部 RPC 报
   `NotAllowed(Anonymous access not allowed)`。**每条连接都要走
   `connect({namespace, database, authentication})`**,不能靠 `use()` 切换
3. **migration 分流** —— X 库有独立的 `schema_version` 记录与迁移序列,
   与笔记库的 1.9.x 序列互不干扰
4. **关闭顺序** —— `before-quit` 要关掉所有连接(参照记忆条目
   `project-graceful-shutdown` 的教训:常驻资源必须有停止调用)

##### 推倒重建:代价与例外

用户允许推倒 X 现有表重建。实测当前数据量:

| 表 | 行数 | 可否重新采集 |
|---|---|---|
| `tweet_inbox` | 4254 | ✓ 7 天窗口,本来就会过期,**丢了无所谓** |
| `search_recipes` | 2 | ✓ 两条 VPN 求助配方(中/英),**手工重建几分钟** |
| **`tweet_feedback`** | **6968** | **✗ 不可再生** |

> ⚠️ **唯一需要保住的是 `tweet_feedback`** —— 它是 40 天连续无断档的人工标注
> (accept/reject 判断 + 128 条 Gemma 原判快照)。这是**你亲手标的、
> 重新采集拿不回来的资产**,也是将来做画像和评估 AI 准确率的唯一底子。
>
> **建议**:重建时把它**导出后迁进新库**(字段可按新模型重整,
> 但数据必须带走)。其余表清空重来无妨。

##### 隔离硬约束(设计期就要守)

- X 库的表**不得**与笔记库的 `atom`/`edge` 有外键、JOIN 或命名空间复用
- X 的 migration 序列独立编号,不混进笔记库的 `schema_version`
- X 的清理策略(TTL、屏蔽后清存量)只作用于 X 库

> 由于 X 表结构本就自洽(不依赖任何 note 表),**独立库不增加设计复杂度**,
> 只是把它们建在 `krig_x` 而非 `krig_note_v2`。
...
### Q8. 存储隔离 —— ✅ 已拍板(2026-09-01)

**用户决定:取 B(独立 database),允许推倒 X 现有表重建。** 详见 §4.1(3.5)。

遗留一个待确认项:**`tweet_feedback` 的 6968 行人工标注要不要迁?**
它是 40 天不可再生的资产(见 §4.1(3.5) 表格)。
*强烈建议迁* —— 其余表清空重来无妨,唯独这张丢了拿不回来。

另:独立库需要先改 `storage/surreal/client.ts`(单例 → 多连接),
**这是全 app 存储底座的改动,会先于 X 需求交付**。请确认接受这个顺序。
...
| 期 | 内容 | 价值 |
|---|---|---|
| **0** | **存储底座**:`client.ts` 单例 → 多连接;建 `krig_x` 库;`tweet_feedback` 迁入 | 需求 ①~⑤ 的前置。**不是 X 功能,是基础设施** —— 但用户已定隔离必须严格,故排最前 |
| **A** | 在 `krig_x` 建 `x_author` / `x_tweet`;采纳即永久 + 去重并入 | **止血**:从今天起不再丢数据、不再重复爬(需求 ①③) |
| **A'** | `x_tweet` 的 `created_at` / `in_reply_to` 落库(抓取脚本已提取,只是没带上) | 3 行改动,但决定了后续能不能区分"推文 vs 回复"、能不能按真实发布时间排序。**越早做,积累的数据越完整** |
| **B** | 屏蔽名单接上数据源(`x_author.blocked` → `accountBlacklist`)+ 屏蔽 UI | 需求 ④,独立可验收 |
| **B'** | 追踪名单:`include:replies` 开关(先 spike)+ 聚合配方 + 名单 UI | 需求 ②⑤(已合并)。依赖 A 期的 `x_author` |
| **C** | 画像聚合 + UI 呈现 + 存量回填 | 需求 ②。**画像具体形态用户说单独再议**,本期只保证素材齐备 |

**0 期是前置**,但它不长(client 多连接 + 建库 + 迁一张表)。

A / A' 期最紧急 —— **每多等一天就多丢一天的采纳正文和回复记录**。
若 0 期意外拖长,可考虑先在现库按 `x_` 前缀落地 A 期止血、再随 0 期整体迁走
(X 表结构自洽,迁移只换库名)—— 但这属于应急路径,默认仍走 0 → A。
