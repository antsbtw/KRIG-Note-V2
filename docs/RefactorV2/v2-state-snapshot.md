...
| 31 | L5-B3.19.1 | **video 字幕引擎(B3.19 第 1 段 / 4)**:V1 直迁字幕 utility(SubtitleCue + parseSubtitleCuesFromText + findActiveCue,~64 行)→ src/drivers/.../blocks/video-block/subtitles.ts;videoBlock attrs 加 `transcript` 字段持久化;NodeView 抽 `appendTranscriptArea` 公共辅助(direct + youtube 共用,Q3=A 一致行为)+ 折叠 textarea 编辑 + 500ms debounce + destroy flush 兜底;cuesCache 闭包就位为 B3.19.2 CC 浮层数据流路径预留;CC 浮层 / Fetch / 翻译留 B3.19.2/3/4 | (待 merge) | 2026-05-08 |
...
| 2026-05-08 | **L5-B3.19.1 video 字幕引擎完成(B3.19 第 1 段 / 4)** ✅:V1 → V2 直迁(~200 LOC + 47 CSS);driver utility subtitles.ts(SubtitleCue + parseSubtitleCuesFromText + findActiveCue 纯函数 0 副作用)+ videoBlock attrs.transcript 持久化(parseDOM/toDOM 双向);NodeView 抽 appendTranscriptArea 公共辅助(direct + youtube 共用,Q3=A 决策一致行为)+ 折叠 textarea(default 折叠避免界面过载,行数显示 `Transcript (N lines) [Show ▾]`)+ 500ms debounce 写 attr + destroy 时 flush 兜底防数据丢失;update() transcript 路径只 recomputeCues 不 paint(避免 textarea 重建丢光标);cuesCache 闭包就位预留 B3.19.2 CC 浮层消费;5 决策点全 A 默认;设计 v0.2 用户审计 P1 修正(direct + youtube 一致行为,3 处文档冲突统一);CC 浮层 / Fetch / 翻译留 B3.19.2/3/4 |
