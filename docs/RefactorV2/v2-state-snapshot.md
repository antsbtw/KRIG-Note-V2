...
## 1. 已 merge 进 main 的阶段(41 个 + 1 待 merge)
...
| 41 | L5-C2 | **ebook PDF 渲染就绪(C2,V1 → V2 ebook 迁移第 2/5 段)**:V1 → V2 直迁(~2207 LOC,2 commit + 装 pdfjs-dist@^4.9.155);**Commit 1 capability**(55df7bf ~1746 LOC):capabilities/ebook-rendering(types 204 / pdf 335 V1 直迁 / fixed-page-content 312 砍 AnnotationLayer / Host.tsx 338 forwardRef + EBookHostHandle 命令式 API + 通过 requireCapabilityApi 拿 library / index 66 + DESIGN 111 + styles 116);**npm 屏障核心** — pdf/index.ts 是唯一 import pdfjs-dist 的文件,view 0 处见;**Commit 2 view**(3d1848a ~461 行新 / -57):EBookToolbar 158 行(V1 305 简版,砍 sidebar / annotation / bookmark / extract / SlotToggle / OpenFilePopup / 锚定锁)+ EBookView 81→213(超 红线 13 行,接 Host + Toolbar handlers + per-ws 状态同步 + 持久化 debounce 500ms 双写)+ ebook.css +97 加 Toolbar 样式 + view install 加 'ebook-rendering' + renderer 显式拉副作用;**订阅模式约定**:view 端订阅 onBookOpened,Host 不订阅(防双订阅重复加载),view 通过 hostRef.loadFromInfo 命令式驱动;capabilities 13 个(原 12 + ebook-rendering);**已知短板**:console getSnapshot warning(C1 残留 V2 既有预存 bug,跟 ebook 无关,留独立 issue)/ EPUB 占位待 C3;typecheck 0 / lint 0 warn / 屏障 grep 通过;**段间不单独验收**(对齐 B3.19 工作流)| (待 merge) | 2026-05-09 |
...
| 2026-05-09 | **L5-C2 ebook PDF 渲染就绪完成** ✅(待 merge,C2 第 2/5 段):V1 → V2 直迁(~2207 LOC,2 commit + 装 pdfjs-dist@^4.9.155);**Commit 1 capability**(55df7bf ~1746 LOC):capabilities/ebook-rendering(types 204 / **pdf 335 V1 直迁** — 唯一 import pdfjs-dist 的文件 / fixed-page-content 312 砍 AnnotationLayer 留 C5 / **Host.tsx 338 forwardRef + EBookHostHandle 命令式 API** + 通过 requireCapabilityApi 拿 library / index + DESIGN + styles);**Commit 2 view**(3d1848a ~461 行):**EBookToolbar 158 行**(V1 305 简版,砍 sidebar / annotation / bookmark / extract / SlotToggle / OpenFilePopup / 锚定锁)+ **EBookView 81 → 213**(超红线 13 行,接 Host + Toolbar handlers + 持久化 debounce 500ms 双写 saveProgress/setReadingState)+ ebook.css +97 + view install 加 'ebook-rendering' + renderer 显式拉副作用;**订阅模式约定**:view 端订阅 onBookOpened,Host 不订阅(防双订阅重复加载);**已知短板**:console getSnapshot warning(C1 残留 V2 既有预存 bug,跟 ebook 无关,留独立 issue)/ EPUB 占位待 C3 / EBookView 213 行略超红线(可拆 hook 瘦身,机会主义)/ Host 内 4 处 V1 直迁 any(pdfjs-dist 类型不全);capabilities 13 个;typecheck 0 / lint 0 warn / 屏障 grep 通过(view 0 处 pdfjs-dist,@capabilities import 全是 type-only);UI 验收待用户实跑 § 3 清单(打开 PDF 见 Canvas 渲染是 C2 核心判据)|
