...
## 1. 已 merge 进 main 的阶段(43 个 + 1 待 merge)
...
| 43 | L5-C4 | **ebook 书签 + EPUB 标注(C4,V1 → V2 ebook 迁移第 4/5 段)**:V1 → V2 改写(~677 LOC + ~70 重写,2 commit);**Commit 1 capability**(af03272 ~579 LOC):新建 hooks/use-bookmarks 103(PDF page bookmarks / EPUB CFI bookmarks 双路,接 host 命令式 + library)+ hooks/use-epub-annotation 132(EPUB 文本选择 5 色高亮,接 host events 推流)+ epub-annotation-picker 78(V1 内联 picker 提取,5 色对齐 V1 #ffd43b/#69db7c/#74c0fc/#b197fc/#ff6b6b);**EPUBRenderer +134**:C3 砍出的 5 API 全补回(onTextSelected / onSelectionDismiss / onAnnotationClick / addHighlight / removeHighlight)+ 3 foliate 事件(setupSelectionListener iframe doc + show-annotation 点击删除 + draw-annotation 自定义颜色 SVG);Host.tsx +55(getCurrentCFI / addHighlight / removeHighlight Handle + onEpubTextSelected / onEpubSelectionDismiss / onEpubAnnotationClick props + loadFromInfo EPUB 路径转推);types/index/styles 同步;**Commit 2 view**(2c3a6e3 ~98):EBookView 280→315(超红线 115)接 useBookmarks + useEpubAnnotation hooks + Cmd+D keymap + Picker JSX + 主区 mousedown 关 picker + **EPUB CFI 持久化补丁**(host.getCurrentCFI → persistEpubProgress);EBookToolbar 299→321 加 ★/☆ 书签按钮(fixed-page + reflowable 各一,黄色 active 态);**C3 § 4.1 已知短板正式 close** — EPUB 重启位置恢复就位;**已知短板**:EBookView 315 超红线 115(机会主义瘦身 use-ebook-keymap / use-ebook-host-state 留 C5 后)/ V1 直迁 11 处 any(foliate-js 类型不全)/ getSnapshot warning(C1 残留独立 issue);capabilities 13 个不变;typecheck 0 / lint 0 warn / 屏障 grep 通过;**段间不单独验收**(对齐 B3.19 工作流)| (待 merge) | 2026-05-10 |
...
| 2026-05-10 | **L5-C4 ebook 书签 + EPUB 标注完成** ✅(待 merge,C4 第 4/5 段):V1 → V2 改写(~677 LOC + ~70 重写,2 commit);**Commit 1 capability**(af03272 ~579 LOC):新建 hooks/use-bookmarks 103(PDF page bookmarks / EPUB CFI bookmarks 双路,接 host 命令式 + library)+ hooks/use-epub-annotation 132(EPUB 文本选择 5 色高亮)+ epub-annotation-picker 78(V1 内联 picker 提取);**EPUBRenderer +134**:C3 砍出的 5 API 全补回(onTextSelected/onSelectionDismiss/onAnnotationClick/addHighlight/removeHighlight)+ 3 foliate 事件(setupSelectionListener iframe doc / show-annotation 点击删除 / draw-annotation 自定义颜色 SVG);Host.tsx +55(getCurrentCFI / addHighlight / removeHighlight Handle + 3 EPUB events props + loadFromInfo 转推);types/index/styles 同步;**Commit 2 view**(2c3a6e3):**EBookView 280→315**(超红线 115)接 useBookmarks + useEpubAnnotation hooks + Cmd+D keymap + Picker JSX + 主区 mousedown 关 picker + **EPUB CFI 持久化补丁**(host.getCurrentCFI → persistEpubProgress);EBookToolbar 299→321 加 ★/☆ 书签按钮(双路 + 黄色 active 态);**C3 § 4.1 已知短板正式 close** — EPUB 重启位置恢复就位;**已知短板**:EBookView 315 超红线 115(机会主义瘦身留 C5 后)/ V1 直迁 11 处 any / getSnapshot warning C1 残留独立 issue;capabilities 13 个不变;typecheck 0 / lint 0 warn / 屏障 grep 通过;UI 验收待用户实跑 § 3 清单(PDF 书签 ★ / EPUB CFI 书签 / EPUB 选区 5 色高亮 / EPUB 重启位置恢复 是 C4 核心判据)|
