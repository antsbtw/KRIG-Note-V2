...
## 1. 已 merge 进 main 的阶段(42 个 + 1 待 merge)
...
| 42 | L5-C3 | **ebook EPUB + Outline + Search(C3,V1 → V2 ebook 迁移第 3/5 段)**:V1 → V2 改写(~1435 LOC + ~109 重写,2 commit + 装 foliate-js@^1.0.1);**Commit 1 capability**(019bd70 ~1038 LOC):capabilities/ebook-rendering 加 epub/index 278(EPUBRenderer V1 366→278 砍标注/留 C4,**唯一 import foliate-js 的文件**)+ epub/foliate-js.d.ts 27 直迁 + reflowable-content 61 + outline-panel 139(V1 99 改写接 host)+ search-bar 108 + hooks/use-search 89(改写接 host);Host.tsx +123(EPUB 分支 + 6 EPUB Handle 方法 + 4 TOC/Search 方法 + onEpubProgressChange + SearchResult 类型);styles +177(reflowable + outline-panel + search-bar);index/types 加 OutlinePanel/SearchBar/useSearch 导出 + Registry api;**Commit 2 view**(6fae98c ~397 行 / -99):新建 use-ebook-progress 76 拆持久化(应对 LOC 红线);EBookView 213→280(超红线 80,加 sidebar+search+EPUB 状态+keymap)+ EBookToolbar 158→299 双模式分支(reflowable 显章节翻页+进度+字号 A−/A+,fixed-page 不变)+ ebook.css +38(body flex row + Toolbar 新元素);keymap useEffect:Cmd+F 开搜索 + EPUB ←/→ 翻章节;**已知短板**:**EPUB CFI 重启不持久化**(Host 没暴露 getCurrentCFI,留 C5 一并补 — 不阻塞,EPUB 重启从头开始)/ EBookView 280 超红线 80(可拆 use-ebook-keymap / use-ebook-toolbar-handlers,机会主义留 C5 后)/ V1 直迁 8 处 any(foliate-js 类型不全)/ getSnapshot warning(C1 残留独立 issue);capabilities 13 个不变(EPUB / Outline / Search 是 ebook-rendering 内部子模块);typecheck 0 / lint 0 warn / 屏障 grep 通过 — view + shell + workspace + slot 0 处 import foliate-js;**段间不单独验收**(对齐 B3.19 工作流)| (待 merge) | 2026-05-10 |
...
| 2026-05-10 | **L5-C3 ebook EPUB + Outline + Search 完成** ✅(待 merge,C3 第 3/5 段):V1 → V2 改写(~1435 LOC + ~109 重写,2 commit + 装 foliate-js@^1.0.1);**Commit 1 capability**(019bd70):capabilities/ebook-rendering 新增 epub/(EPUBRenderer 278 行 V1 366→278 砍标注/留 C4,**唯一 import foliate-js 的文件**)+ epub/foliate-js.d.ts 27 直迁 + reflowable-content 61 + outline-panel 139(V1 99 改写接 host)+ search-bar 108 + hooks/use-search 89(改写接 host);Host.tsx +123(EPUB 分支 + 6 EPUB Handle 方法 + 4 TOC/Search 方法 + onEpubProgressChange + SearchResult 类型);styles +177;index/types 加 OutlinePanel/SearchBar/useSearch 导出 + Registry api;**Commit 2 view**(6fae98c):新建 use-ebook-progress 76 拆持久化(应对 LOC 红线);**EBookView 213→280**(超红线 80)+ EBookToolbar 158→299 双模式分支(reflowable 显章节翻页+进度+字号 A−/A+;fixed-page 不变)+ ebook.css body flex row + toolbar 新元素;keymap:Cmd+F 开搜索 + EPUB ←/→ 翻章节;**已知短板**:**EPUB CFI 重启不持久化**(Host 没暴露 getCurrentCFI,留 C5 一并补 — 不阻塞,从头打开)/ EBookView 280 超红线 80(机会主义瘦身留 C5 后)/ V1 直迁 8 处 any(foliate-js 类型不全)/ getSnapshot warning C1 残留独立 issue;capabilities 13 个不变(EPUB/Outline/Search 是 ebook-rendering 子模块);typecheck 0 / lint 0 warn / 屏障 grep 通过 — view 0 处 foliate-js;**段间不单独验收**;UI 验收待用户实跑 § 3 清单(打开 EPUB 见 foliate-view 渲染 + Outline + Cmd+F 搜索是 C3 核心判据)|
