...
| 37 | L5-B3.19.c | **video-block Memory Playback Mode(B3.19 第 3/5 段)**:艾宾浩斯间隔重复(~310 行新增 / ~6 行重写);actions/memory-button.ts(265 行 generator + 状态机 + ⏮🧠⏭ + 30/60/90/120s dropdown,对齐 V1 序列 [0]→[0,1]→[1,2]→...);段边界检测订阅 time-tracker.onTimeUpdate(0.3s buffer Qc-2=A);duration 未就绪(YouTube 异步)显 🧠 ⏳ + onDurationReady 回调启动(Qc-4=A);spec.ts 加 segmentDuration(default 60)+ memoryLastStep(default 0)attrs;active 不持久(对齐 V1,重启不自动播);**actionBar 顺序一次对齐 V1**(Qc-6=A):CC | ⏮🧠⏭ | 🌐 | 📝 | ⛶(下载留 e 段,vocab 留 d 段);7 个 Qc 子决策全 A 默认;无 CSS 新增(memory-wrap 内联 position:relative + flex,内部按钮复用 __action-btn,dropdown 复用 a 段 .krig-video-block__dropdown 自动锚定父 wrap)| (待 merge)| 2026-05-09 |
