...
| 40 | L5-C1 | **ebook 平台基座 + library capability + view 骨架(C1 启动,V1 → V2 ebook 迁移第 1/5 段)**:V1 → V2 直迁(~2747 LOC,2 commit);**Commit 1 platform**(89ae4ef ~1047 LOC):src/platform/main/ebook/{bookshelf-store JSON atomic write / annotation-store / file-loader / library-handlers}(D-3=B v0.3 修订:V2 无 SurrealDB 客户端,改 JSON 起步,沿用 learning/vocab-store / media-storage 模板)+ shared/ipc/channel-names 加 25 EBOOK_* + preload + electron-api 类型 + ipc-bus 接 registerEBookHandlers;广播改 BrowserWindow.getAllWindows();入参严格 typeof 校验;**Commit 2 capability+view**(d8389fa ~1700 LOC):capabilities/ebook-library/(types/index/DESIGN,扁平 27 method API,W5 严格态 A 边界双导出)+ views/ebook/(index 39 / EBookView.tsx **81 行 ≪ 红线 150~200** / data-model 161 + nav-side-content 640 含完整书架 + 8 项右键菜单 + ImportModal + toast / bookshelf-commands 145 ebook-view.* 9 commands + 桥接器 / ebook.css 73 薄壳)+ renderer/index.tsx 接 import '@capabilities/ebook-library' + '@views/ebook';**view-id 命名微调**:v0.3 字面 'ebook' → 实际 'ebook-view'(对齐 V2 既有 note-view / web-view 惯例,目录仍 views/ebook/);D-2=A pluginStates['ebook-view'](activeBookId / expandedFolders / readingState);D-7=A annotation thumbnail base64 inline;D-12=A OCR 砍出;**砍/推迟**:Application Menu Open eBook 留 C2/C5 / 锚定同步 D-9=B 单独阶段 / PDF 全书提取 D-8=A 不在本迁移 / Outline+Search+FixedPageContent+ReflowableContent 留 C2~C3 / OCR 留 C5;capabilities 12 个(原 11 + ebook-library);typecheck 0 error / lint 0 warn;UI 验收待用户 npm start 实跑 § 3 清单;**段间不单独验收**(对齐 B3.19 工作流),C5 后给完整整体清单 | (待 merge) | 2026-05-09 |
