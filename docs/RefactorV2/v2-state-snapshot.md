...
| 45 | L5-C6 | **PDF 提取 → Note(C6,ebook 迁移补段,原 D-8=A 不在迁移补做)**:V1 → V2 改写(~1442 LOC,4 commit);**Commit 1 platform**(7edbecb ~488 LOC):extraction/config + upload-service(168 V1 直迁,multipart + JWT + 401 自刷)+ extraction-handler(138 V1 直迁,download intercept JS:`<a>` blob → fetch → console.log('KRIG_IMPORT:'+JSON))+ handlers(143,EXTRACTION_UPLOAD/IMPORT IPC + did-attach-webview hook + 广播 EXTRACTION_NOTE_CREATE);3 channel + preload + electron-api 类型 + ipc-bus + index 挂载;**Commit 2 text-editing**(97e7e64 ~639 LOC):**atoms-to-pm 482**(13 atom type 全覆盖 + 5 InlineElement kebab→camelCase 归一 + tiptapContent 直装 + List tree builder flat+parentId→nested + image base64→media:// 走 capability registry)+ **sanitize-atoms 157**(契约 § 9 八条容错:v1→v2 类型迁移 / document root 过滤 / parentId 清理 / sourcePages 迁 from / 空 text 过滤 / mathInline 归一);**Commit 3 view**(b9bfcec ~315 LOC):**extraction-import 178**(view/note 内,batch → folder.find/create + 每章 atoms→sanitize→atomsToProseMirror→DriverSerialized→noteStore.create + 同名同夹去重)+ **use-extraction-import 28**(NoteView 内挂,onExtractionNoteCreate → import);**EBookView +28**(handleExtract 调 IPC + commandRegistry.execute('web-view.open-url'),不直 import @views/web)+ **EBookToolbar +16**(📤 按钮 fixed-page only)+ TextEditingApi 暴露 atomsToProseMirror/sanitizeAtoms + AtomInput/PMDocNode 共享类型;**架构对齐**:converters 全在 capability text-editing(view 经 cap registry),extraction-import 在 view/note(只 note 直访问 noteStore/folderStore),跨 view 走命令路由;**已知短板**:进度无 UI 反馈(console.log 起步)/ Platform URL 硬编码内网 192.168.1.240 / 凭证 admin/123456 写死 / image 大 base64 走 IPC 阈值未测;**ebook 段彻底闭环**:C1~C6 全完整,V1 → V2 ebook 迁移收官;capabilities 13 不变;typecheck 0 / lint 0 warn | 68f7d2c | 2026-05-10 |
| 46 | L5-G1 | **V1 → V2 graph 迁移第 1 段(共 5 段 G1~G5):平台基座 + library-store + view 骨架**(~2001 LOC driver + 122 DESIGN.md,3 commit + 1 design 修订);**Commit 1 platform**(466021f ~628 LOC):src/platform/main/graph/(canvas-store 366 行 JSON atomic write + folder ops 合并 G1-5;library-handlers 149 行 14 IPC — plan 20 砍 6:GRAPH_PENDING_OPEN / GRAPH_OPEN_IN_VIEW / GRAPH_DELETED / GRAPH_TITLE_CHANGED / GRAPH_SET_ACTIVE / GRAPH_GET_ACTIVE,统一走 GRAPH_LIST_CHANGED 单一推送 + commandRegistry 同 renderer 路由替代;index 16 行 re-export);channel-names + electron-api + preload + ipc-bus 接入 +97 行;**Commit 2 capability + view**(2521466 ~1373 LOC):capabilities/graph-library-store(types 85 + index 171 + DESIGN 122,双导出 + capabilityRegistry.register);views/graph-canvas-view(7 文件:index 48 / GraphCanvasView 66 ≪ 红线 200 / Toolbar 67 / data-model 148 / nav-side-content 418 ebook 模板精简 35% — 删 ImportModal/pickFile/relocate/transferToManaged/openFailed / canvas-commands 161 含 8 commands + 3 trigger 桥 / CSS 85);renderer/index.tsx +2 行;**用户拍板决策**:D-1=A `graph-canvas-view`(对齐 V2 既有 `*-view` 惯例,与 ebook v0.3 → 实施同模式微调)/ D-2=A pluginStates / D-3=B JSON 起步(对齐 ebook D-3=B 模板)/ D-4=B 过渡态 platform/main/graph,退出条件二合一(G5 验收 + 稳定 ≥2 周 + W6 SurrealDB 客户端 epic);**G1 决策细化 12 项 G1-* 全 A 默认**(G1-4 持久化双文件 canvases.json metadata + documents/{id}.json,G1-5 folder 合并到 canvas-store,G1-8/G1-9 砍 IPC);**design v0.1 → v0.2 实施后用户 P2 复审**(d73dbe3):文档内部数字一致性修订(canvas-store 估 ~180 / 实 366;IPC 计数全文统一 14 条;Commit 拆分估/实双列);**屏障 grep 0 命中**(view 0 import three / view 0 运行时 import @capabilities);capabilities 13 → 14;**用户验收 12 项全过**(创建/重命名/复制/移动/删除/重启恢复/多 ws 隔离);typecheck 0 / lint 0 warn | 7cb79c6 | 2026-05-10 |
| 47 | L5-G2 | **V1 → V2 graph 迁移第 2 段(共 5 段 G1~G5):shape-library capability**(~1115 LOC driver + 113 DESIGN.md + 27 JSON 资源,6 commit:2 design + 1 feat + 3 fix);**Commit 主体 feat**(37faace ~1115 LOC):capabilities/shape-library/(types 298 V1 types.ts 333→298 拆 Instance 系 / index 150 双导出 + capabilityRegistry.register + side-effect bootstrap + alive 行 / shapes:registry 46 + 22 JSON 直迁 basic 11/arrow 3/flowchart 4/line 3/text 1 + renderers/parametric 110 V1 renderParametric→evaluateShape 改输出 EvaluatedPath / renderers/formula-eval 207 V1 纯数学直迁 / bootstrap 31 + __smoke__/run 99 / substances:registry 47 + 5 JSON 直迁 library 2/family 3 + composer 21 + visual-rules 21 空壳 G2-7=B + bootstrap 31);**用户拍板决策**:G2-1=A `shape-library` id / G2-2=A EvaluatedPath 纯数据(P1-1 屏障核心,types.ts 0 含 'three-mesh'/'composite'/THREE.* 字面量)/ G2-3 G2-4=A 函数命名 evaluate evaluateShape / G2-7=B composer 空壳 / G2-8=B side-effect 立即 bootstrap / G2-10=B Instance 不搬到 shape-library;**P1-1 严格版屏障核心**:shape-library 真代码 0 import three / 0 引用 THREE.*(grep 验证);path-to-three.ts(V1 395 行)留 G3 一起搬到 canvas-rendering/scene/;**P1-A install 列表 G2 启动前修订**(ba10e60):用户审计 G2 design v0.1 与上游 plan v0.2 § 6.1 口径冲突 → 选 A 对齐 plan 全文一致:src/views/graph-canvas-view/index.ts install 1→4 项完整声明(graph-library-store/shape-library/canvas-rendering/canvas-text-node)+ G1 design v0.2→v0.3 / G1 completion / G2 design v0.1→v0.2 五处文档同步;**P2 顺手修补 3 处**(f2f2c32+19fecbf+73f9cb6):①graph data-model.ts hydrate selectedIds 兜底 → DEFAULT_WS_STATE.selectedIds(React 19 dev mode getSnapshot stable ref);②shape-library/index.ts 加 dev-only window.__krig.shapeLib 桥 + 启动自动 smoke(DevTools 自检用);③renderer/index.tsx dev hook 改 spread 不覆盖(初始化顺序 bug);④顺手修 NoteView + EBookView 同类 V2 既有 bug(selectedIds 兜底引用不一致);capabilities 14 → 15;**用户验收 6 项 + 1 P2 全过**;typecheck 0 / lint 0 warn / 屏障 grep 0 命中;**遗留**:install-coverage / viewTypeRegistry warn P1-A 渐进式 install 下预期,G3 / G4 渐次归零 | 67e81f6 | 2026-05-10 |
