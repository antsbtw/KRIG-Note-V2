...
| dev hook 桥(`window.__krig.shapeLib`) | v0.1 设计 § 7 第 4 项写"runShapeSmoke() 挂在 index.ts 末尾的 dev-only 分支" | 实施 + commit 4 / 5 修补:挂 + 处理 renderer 后续覆盖问题 |
...
| ✅ smoke test 通过 | `runShapeSmoke()` 返回 ok:true, total:22, byCategory 完整对齐设计 | ✅ |
