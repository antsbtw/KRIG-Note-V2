...
> v0.3 · 2026-05-09 · C1 启动前实施侧前置阻塞修订(D-3 改 B + D-4 退出条件升级)
>
> 配套文档:
...
src/platform/main/ebook/              ← 平台(主进程实现)
   bookshelf-store.ts                 V1 直迁(D-3=B JSON 实现,模板 platform/main/learning/vocab-store.ts)
...
| **eBook 持久化:JSON 还是 SurrealDB** | 决策点 D-3 | C1 前置 | V1 两套并存(`bookshelf-store.ts` JSON 321 行 + `bookshelf-surreal-store.ts` SurrealDB 389 行,实际生效后者);v0.3 修订:V2 此刻 0 SurrealDB 客户端(storage/ 仅 README,V1 client.ts 312 行 + schema/init/migrate 共 ~1100 行),C1 引入会失控 → **D-3 改 B(JSON 文件)**,沿用 `learning/vocab-store.ts` 模板;V1 SurrealDB 实现保留作 C5 后期 SurrealDB 客户端 epic 起点 |
...
| V2 文件 | 来源 | 备注 |
|---|---|---|
| `bookshelf-store.ts` | V1 `src/main/ebook/bookshelf-store.ts` 直迁(JSON 实现,321 行) | **D-3=B(v0.3 修订)**:V2 此刻 0 SurrealDB 客户端,沿用 `learning/vocab-store.ts` JSON atomic write 模板。文件路径:`{userData}/krig-note-v2/ebook/bookshelf.json` |
| `annotation-store.ts` | V1 `src/main/ebook/annotation-store.ts` 直迁(JSON 实现,98 行) | 同上;路径:`{userData}/krig-note-v2/ebook/annotations/{bookId}.json` |
...
| **D-3** | bookshelf / annotation 持久化后端 | SurrealDB(V1 实际生效的实现 `bookshelf-surreal-store.ts` 直迁) | **JSON 文件**(V1 兜底实现 + EBookView 设计 v2 § 1.3 写 "Phase 1 用 JSON,Phase 2 迁 SurrealDB",对齐 V2 既有 `learning/vocab-store.ts` / `media-storage` 的 JSON 起步模式) | **v0.3 修订:推荐 B**。<br>原 v0.2 推荐 A 的隐含假设是「V2 已有 SurrealDB 客户端」,实施时发现:**V2 `src/storage/` 仅 README,无任何 SurrealDB 客户端**;V1 客户端是 `src/main/storage/` 整套 ~3163 行(client 312 + schema 146 + init-docs 326 + migrate 273 + 6 个 store 等),C1 一并引入会失控。<br>**B 路径退出条件**:① C5 验收通过 + ebook 主功能稳定 ≥ 2 周;② 独立的 SurrealDB 客户端落地 epic 完成(候选 W6);③ 两条达成后启动「ebook JSON → SurrealDB」迁移子段。<br>**保留资产**:V1 `bookshelf-surreal-store.ts` + `annotation-surreal-store.ts` 不删除,作为后期升级时的参考实现 |
| **D-4** | 持久化实现归位(随 D-3=B 升级语义) | 直接放 `src/storage/ebook/`(对齐 charter directory-structure § 1)| **过渡态:先放 `src/platform/main/ebook/`(JSON 实现),后续合并至 `src/storage/ebook/`(SurrealDB 实现)**| A 是字面合规;B 是 V1 同位、迁移阻力小。**推荐 B**(与 D-3=B 配套)。<br>**钉死的过渡约束**(v0.3 升级):<br>① **过渡性质**:`src/platform/main/ebook/` 内的 JSON store 是显式临时实现 + 临时位置,**不是终态**<br>② **退出条件二合一**(必须全部满足):<br>&nbsp;&nbsp;a. C5 验收通过 + ebook 主功能稳定运行 ≥ 2 周<br>&nbsp;&nbsp;b. 独立的 SurrealDB 客户端 epic 落地(client + schema + init-docs + migrate-json-to-surreal 等基础设施迁入 `src/storage/`)<br>③ **目标落点**:`src/storage/ebook/{bookshelf-store, annotation-store}.ts`(SurrealDB 实现),候选阶段 **W6 storage 层落地** — 该阶段把 ebook 作为 storage 层第一个真用例,顺带把 V1 storage 模块整体迁过来(单独完整 epic)|
...
| `surrealdb` | **不引入**(D-3=B v0.3 修订)| 本迁移走 JSON 文件,sdk 留给 W6 storage 层 epic |
| `node:fs` / `node:path` / `electron.app.getPath` | platform/main/ebook(JSON 文件 atomic write) | renderer 0 import,沿用 `learning/vocab-store.ts` 模板 |
...
| D-3 持久化后端 | **B JSON 文件**(v0.3 修订:V2 此刻无 SurrealDB 客户端,沿用 learning/vocab-store 模板) | ✅ |
| D-4 持久化归位(随 D-3=B 升级语义) | B 过渡态 platform/main/ebook(JSON)→ 退出条件二合一(C5 验收 + 稳定 ≥2 周 + W6 SurrealDB 客户端 epic 落地)→ 目标落点 src/storage/ebook/(SurrealDB 实现)| ✅ |
| D-5 文件不存在重新定位 | A 完整流程 | C1 后可定 |
| D-6 annotation 升格 Atom | A 不升格 | ✅ |
| D-7 thumbnail 存储 | A base64 inline | ✅ |
| D-8 「提取」按钮 | ~~A 不在本迁移~~ → **C6 补做完成**(2026-05-10,见 [stages/L5C6-pdf-extraction-completion.md](./stages/L5C6-pdf-extraction-completion.md))| ✅ |
...
| 2026-05-09 | v0.3 | C1 启动前实施侧前置阻塞修订(用户拍板 A2 路径):**D-3 改 B(JSON 文件)** —— 实施时发现 V2 `src/storage/` 仅 README,无 SurrealDB 客户端(V1 客户端 ~3163 行),C1 顺手搭建会失控。沿用 V2 既有 `learning/vocab-store.ts` JSON atomic write 模板,与 V2「平台基础设施增量主义」节奏对齐。**D-4 退出条件升级为二合一**:C5 验收 + 稳定 ≥2 周 + 独立 SurrealDB 客户端 epic(候选 W6 storage 层落地)落地后,统一升级为「JSON → SurrealDB + platform → storage」。V1 SurrealDB store 实现保留作 W6 epic 起点参考。同步更新 § 2.2 / § 3.1 表 / § 3.4 表 / § 4 D-3 D-4 / § 6.3 npm 屏障 / § 9 总览 共 6 处 | wenwu + Claude |
| 2026-05-10 | v0.4 | C5 merge 后用户决议「D-8 必须做,否则 PDF→Note 链路缺关键一环」,新增 C6 段补做(分支 `feature/L5C6-pdf-extraction`):main 侧基建 + atoms-to-pm/sanitize-atoms 转换器 + view 端 📤 + import receiver。架构对齐 V2 三大原则(转换器在 capability,跨 view 走命令路由,extraction-import 在 view/note 内只 note 直访问 noteStore/folderStore)。决策落点:E-1~E-7 全 A(用户「同意你的建议」拍板)。详见 [stages/L5C6-pdf-extraction-completion.md](./stages/L5C6-pdf-extraction-completion.md) | wenwu + Claude |
