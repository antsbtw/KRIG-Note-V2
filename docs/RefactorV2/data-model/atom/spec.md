...
#### 2.5.1 L7 block atomization 字面承接(2026-05-21 反向更新)

字面[decision 026](../persistence/decisions/026-block-atomization-sub-phase-design.md) 字面拍板:

- **pm atom 字面颗粒度从"整篇 doc"字面升级为"单 block"**(对齐 §0.1 字面"语义最小单元"语义)
- 字面 §2.2 字面 "Block —— pm Atom 的语义组合类型" 字面**反向**:Block 字面**就是** pm Atom 本身,不再是"组合类型"
- 字面拆 atom 字面范围:叶子 + 叶子级容器(§3.1.1)— 22 NodeSpec(详 D-04 修订)
- 字面**不拆**字面结构性容器:table / tableRow / bulletList / orderedList / taskList / columnList(§3.1.2)
- 字面嵌套字面通过 user:krig:childOf 边表达(跨结构性容器跳层,§6.1)
- 字面顺序字面通过 user:krig:nextSibling 边链表表达(§6.2)
- 字面归属字面通过 user:krig:belongsToNote 边表达(§6.3)

**字面 inline 节点字面不拆**(§3.1.3): text / hardBreak / fileLink(inline)/ noteLink / mathInline 等字面仍嵌套在所属 block atom 的 `PmPayload.content` 内。

**字面 sub-phase 完成 commits**:`feature/L7-block-atomization` 字面 20+ commits(详[完成报告](../../notes/block-atomization-completion-report-2026-05-21.md))。
