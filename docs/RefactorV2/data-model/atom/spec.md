...
#### 2.5.1 L7 block atomization 承接(2026-05-21 反向更新)

[decision 026](../persistence/decisions/026-block-atomization-sub-phase-design.md) 拍板:

- **pm atom 颗粒度从"整篇 doc"升级为"单 block"**(对齐 §0.1 "语义最小单元"语义)
- §2.2 "Block —— pm Atom 的语义组合类型" **反向**:Block **就是** pm Atom 本身,不再是"组合类型"
- 拆 atom 范围:叶子 + 叶子级容器(§3.1.1)— 22 NodeSpec(详 D-04 修订)
- **不拆**结构性容器:table / tableRow / bulletList / orderedList / taskList / columnList(§3.1.2)
- 嵌套通过 user:krig:childOf 边表达(跨结构性容器跳层,§6.1)
- 顺序通过 user:krig:nextSibling 边链表表达(§6.2)
- 归属通过 user:krig:belongsToNote 边表达(§6.3)

**inline 节点不拆**(§3.1.3): text / hardBreak / fileLink(inline)/ noteLink / mathInline 等仍嵌套在所属 block atom 的 `PmPayload.content` 内。

**sub-phase 完成 commits**:`feature/L7-block-atomization` 20+ commits(详[完成报告](../../notes/block-atomization-completion-report-2026-05-21.md))。
