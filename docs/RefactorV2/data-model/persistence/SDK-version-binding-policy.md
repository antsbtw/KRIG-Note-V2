...
| 2026-05-16 | v1.9 | sub-phase 024 callout Upload tab **字面零新增 SDK 备案**:消费既有 `media-storage` capability(audit Wave 3.1 已 capability 化,无 `@storage/*` 跨层穿透)+ 既有 `emoji-mart@5.6.0` / `@emoji-mart/data@1.2.1` / `lucide-react@1.14.0`,字面零 npm install 新包。**字面无 §4 表变更**,字面仅 §6 时间线登记。授权依据:[decision 024 §0.5.2](decisions/024-callout-upload-tab.md) 字面 "本 sub-phase 字面零新增 SDK,字面不触发跨大版本升级,字面不触发 SDK-policy §2 独立 sub-phase 拆分要求" | [decision 024](decisions/024-callout-upload-tab.md) |
