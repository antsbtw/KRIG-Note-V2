...
| **tableCell / tableHeader** | 叶子级容器(单元格)| 用户标注表格某格;tableHeader 字面定位为叶子级以便单元格统一处理(用户拍板拒拆 tableHeader 与该原则字面冲突 — 见 §3.1.2 注 1)|
...
| 类型 | 字面 | 不拆理由 |
|---|---|---|
| **table** | 表格根容器 | 用户从不单独引用整表(可由 caption 块代替);table 内 tableRow / tableCell 通过 childOf 边拼装 |
| **tableRow** | 表格行 | 用户从不单独引用行;通过 childOf 边表达"cell 属于哪个 row" 由 capability 层在拼装时**直接跳过中间层**,tableCell.childOf 指向 table atom 而非 tableRow(详 §6.1) |
...
| **columnList** | 多列容器 | 同 listList 思路;只拆 column,不拆 columnList |

> **注 1**: 关于 tableHeader 是否拆 atom — 字面歧义可能存在(tableHeader 介于"行"和"单元格"之间)。**临时默认**:tableHeader 与 tableCell 同模式拆 atom(都是叶子级单元格),tableRow 不拆。实施任务设计 Stage 1 复 grep verify schema 关系后再确认。
...
按本规则,100 行 × 10 列 table 的 atom 负载:

- 1 table atom(根容器,不拆)→ table.content = []
- 0 tableRow atom(不拆)
- 100×10 = 1000 tableCell atom
- 每 cell 内 ≥1 paragraph atom → 1000 paragraph atom
- **合计:1001 atom + ~1000 childOf 边(cell.childOf → table)**

(原"全部 PM block"拍板:1101 atom + 1100 childOf 边 + tableRow 100 atom — 现砍掉中间层)
...
// 例 3:table(嵌套场景 — tableRow / tableCell / cell内 paragraph 都是独立 atom)
{
  domain: 'pm',
  payload: {
    type: 'table',
    attrs: { id: '<ULID>' },
    content: []   // 同上,tableRow 通过 childOf 边关联
  }
}
```

**容器 block(listItem / tableCell / callout / column / blockquote 等)的 `content` 字段 = 空数组**,内部嵌套通过 `user:krig:childOf` 边表达(详 §6.3)。
...
**结构性容器跳层规则**(§3.1 拍板"tableRow / 3 list 容器 / columnList 不拆"的必然推论):
...
| tableCell 在 tableRow 内,tableRow 在 table 内 | tableCell.childOf → table atom(跳过 tableRow)|
