...
3. **不动其他已完成模块对外契约**:
   ...
   - `src/storage/` 一律不动
   - 例外:`src/platform/main/folder/capability-impl.ts` 的 `listFolders / createFolder` 改造(本决议核心改造点)+ `src/capabilities/folder/types.ts` 加 `viewType` 入参
...
- **§7 clearAll migration**:废弃 `storage.clearAll()` 调用(API 不存在),改用"实施期 main 端启动早期直接调 [client.ts](../../../../../src/storage/surreal/client.ts) `getDB().query('DELETE atom; DELETE edge;')` 单语句 wipe + 错误处理"(SurrealQL 单语句字面 sub-phase 3a-tx 已 binary verify 原子)
...
### Step 5.2 — types + capability 接口签名扩展
...
**完成判据**:typecheck 通过(但 4 caller 处会暴露 TypeScript error,Step 5.5 修)。
...
4 caller 处 TypeScript 错误留 Step 5.5 修。
ebook viewType 未加,留 sub-phase 022 (§4.3 兼容约束)。
```

### Step 5.3 — main 端 listFolders + createFolder 改造

**任务**:
1. 改 [folder/capability-impl.ts](../../../../../src/platform/main/folder/capability-impl.ts) `listFolders` / `createFolder` 实施按 §4.1 草案
2. 改 [folder/handlers.ts](../../../../../src/platform/main/folder/handlers.ts):FOLDER_LIST / FOLDER_CREATE handler 透传 viewType 入参

**完成判据**:typecheck 通过 + verify 脚本(Step 5.1)PASS。

**commit message**:
```
feat(folder/main): listFolders + createFolder 加 viewType 过滤/写边 (decision 021 §4.1)
...
2. graph 路径 IPC channel 字段**不**加 viewType(renderer 透明)
3. canvasStore.folderList / folderCreate 字面对齐

**完成判据**:typecheck 通过 + nav-side-content.tsx 字面 0 改动(graph 路径透明)。
...
### Step 5.5 — 4 caller 改造 + Q7 弱保护

**任务**:
1. note 3 caller 改造:`folder.listFolders('note')` / `folder.createFolder(title, parentId, 'note')`
   ...
   - (其他 grep 出现处一并字面对齐)
2. Q7 弱保护:`deleteFolder` 前预查含资源数,UI 弹框确认
   - main 端 deleteFolder 函数返 `previewCount: { folders, resources }` 给 UI(或独立 method)
   - renderer 端弹框组件(NavSide / nav-side-content)接 confirm,确认后才走 deleteFolder

**完成判据**:typecheck 全 PASS + UI 弹框集成测试通过

**commit message**:
```
feat(note + folder + UI): 4 caller 加 viewType + Q7 弹框 (decision 021 §1.1 + §5.5)

note view 3 caller 改 'note' viewType,graph nav-side 字面 0 改动 (走 folder-adapter)。
deleteFolder 前 main 端返 previewCount,UI 弹框确认含资源数后才删。
...
2. clearAll 实施:`DELETE atom; DELETE edge;`(单 statement 走 SDK 原生 transaction sub-phase 3a-tx)
...
> ⚠ **§0.4 第 8 项自审命中**:`StorageAPI` 字面**无 `clearAll`** 方法,本决议不引入新接口(避第 5/11 次教训"假设已实施模块自动支持新需求")。改用 [client.ts](../../../../../src/storage/surreal/client.ts) `getDB().query()` 直跑 SurrealQL 单语句 wipe,沿 sub-phase 3a-tx beginTransaction Path 1(decision 020 §3.5.ter)单语句原子语义。
...
| `docs/RefactorV2/data-model/persistence/SDK-version-binding-policy.md` | §2.2 第 6 / 7 步 | 第 6 步加"启动包 / 总纲文档字面区分现状 vs 未来计划"(第 14 次);第 7 步加"决议引用 storage API / 类型字段前必须 grep 真实接口字面 + LiteralValue 字面三字段 `{ kind, type, value }` 缺一不可"(第 15 次)|
