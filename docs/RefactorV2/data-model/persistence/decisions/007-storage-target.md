...
V1 用 Sidecar 模式（spawn 独立 binary）。**V2 sub-phase 1 实施结果**：

**EM1 硬门槛在 sub-phase 1 实施时触发** —— `surrealdb` npm 包是 client SDK 不含 Embedded engine，Embedded 需独立 `@surrealdb/node` 包；但 `surrealdb@2.x` (client) 与 `@surrealdb/node@3.x` (engine) 主版本不兼容（3.x client 仍是 alpha）。

→ **sub-phase 1 实际采用 Sidecar only 模式**（commit `b5b2a29` Step 5.6）。

**实施模式**：
- ✅ **Sidecar 模式**（已实施于 sub-phase 1）—— 复用 V1 完整路径（`src/main/storage/client.ts` 含孤儿清理）。binary 走 `app.getPath('userData')/bin/surreal` + Homebrew fallback。
- ⏳ **Embedded 模式** —— 留未来 sub-phase（待 `surrealdb@3.x` client SDK 转 stable + Embedded engine 包成熟后独立 sub-phase 切换）

历史 RFC 假设（"Embedded 优先 / Sidecar fallback"）**事实纠错为"Sidecar only，Embedded 留未来"**。下方 §4.2.1 硬门槛清单作为未来切 Embedded 时的判定标准保留。
