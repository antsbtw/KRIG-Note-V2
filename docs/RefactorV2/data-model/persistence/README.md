...
> **分支**: `feature/L7-persistence-spec`
>
> ⚠ **本目录所有文档处于 RFC 阶段**：含"临时默认"、"待决议"标识的字段 / 字段名 / 选型均**非最终规范**。
> RFC 转正条件：Phase 3a-3d 全部完成 + audit 通过 + 用户拍板。在此之前，**实施代码遇本目录与现有实现冲突时以现有实现为准**（参考 [`naming-conventions.md §8`](../naming-conventions.md) RFC 生效流程模式）。
