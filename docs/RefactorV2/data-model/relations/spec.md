...
3. **subject / object 可以是 atom 或 literal**：边不一定连两个 atom（属性边的 object 常是 literal 值）。
...
- **endpoint 可以是 atom 或 literal**：边不强制连两个 atom。`person-atom user:family-tree:hasGender 'female'` 中 object 是 literal，不需要为 'female' 单独建一个 atom。
...
| LiteralValue | AtomRef | 反向属性边（罕见，仅在特殊场景） |
| LiteralValue | LiteralValue | **禁止** —— 边必须至少有一端是 atom |
...
| 编号 | 问题 | 临时默认 | 解决时机 |
|---|---|---|---|
| E1 | 反向 literal-as-subject 边（如 `'female' literal isGenderOf <atom>`）是否允许？还是统一改 subject 为 atom？ | **默认禁止** —— subject 必须是 atom，方向不便时改写边语义为 atom-side-subject。 | Phase 2 视真实需求 |
