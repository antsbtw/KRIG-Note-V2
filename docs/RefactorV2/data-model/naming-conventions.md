...
| `alt` | ✗（V1 image 没有） | 1 | **新增字段** | Markdown `![alt](src)` 标准；alt = "替代文本"（图片无法显示时的文字），供屏幕阅读器 / 加载失败场景。**与 caption 并存而非替换**。 |
| `caption` | ✓ | 2 | **保留语义不变** | HTML5 `<figcaption>` 标准；caption = "可见说明文字"（图片下方/旁的说明）。图片节点可同时有 alt 和 caption，语义不同。 |
| `width` / `height` | ✓ | 2 | 保留 | HTML `<img width height>` 标准 |
| `mimeType` | ✓ | 2 | 保留 | HTML `type` 属性 + 通用约定 |
| `size` | ✓ | 3 | 保留 | KRIG 自定义（字节数）；HTTP Content-Length 概念 |
| `poster` | ✓ | 2 | 保留 | HTML `<video poster>` 标准 |
| `duration` | ✓ | 2 | 保留 | HTML media element duration 属性 |
| `originalSrc` | ✓ | 3 | **删除**（待决议）| V1 历史包袱（保存外部 url 防止 media store 失败时回退）；V2 提议走边表达"originalSourceUrl" 关系；**有 V1 历史数据迁移风险，单独决议** |
| `filename` | ✓ | 3 | 保留 | KRIG 自定义（HTTP `Content-Disposition` 概念） |

**关键说明（caption vs alt）**：

V1 image 节点用 `caption` 字段，**实际上常被用作 alt 也常被用作 caption**，语义混乱。HTML5 / Markdown 标准里两者完全不同：

| 字段 | 用途 | 标准来源 |
|---|---|---|
| `alt` | **替代呈现** —— 图片无法显示时的纯文本替代 | Markdown `![alt](src)` / HTML `<img alt>` |
| `caption` | **可见说明** —— 图片下方 / 旁边的视觉化说明文字 | HTML5 `<figcaption>` |
| `title` | tooltip / 鼠标悬停 | Markdown `(... "title")` / HTML `title` 属性 |

V2 image 节点**三个字段并存**：
- `alt` **推荐必填**（按 Markdown 标准 / 无障碍最佳实践）
- `caption?` 可选（HTML5 figure 概念）
- `title?` 可选（Markdown / HTML 标准）

**关于 alt 的必填严格度**：
- **新建 image atom 时**：推荐必填（capability.text-editing 可在 UI 引导补齐）。
- **存量迁移期**：允许为空（V1 历史数据 caption 字段拆分到 alt 时，部分情况可能没有合适的 alt 文本可填）。
- **导出 / 渲染前**：建议补齐（防止可访问性问题），由对应能力检查。

→ V1 历史数据如有 `caption`，迁移时需人工判断该填到 alt 还是 caption（具体场景具体决定）。**仅本 RFC 是提案，没有自动迁移**。
...
| `bgColor` / `color` | ✓（mathBlock） | — | **保留待决议** | 视图样式倾向归视图层；但 mathBlock 历史依赖，单独决议（见 §6 N4） |
...
### 2.12 Mark 类型命名（争议项 — 待决议）

| V1 命名 | 阶梯 | V2 提案 | 决议状态 |
|---|---|---|---|
| `bold` | 1 | **待决议**（提案：改 `strong`） | 单独决议 —— 见 §6 N6 |
| `italic` | 1 | **待决议**（提案：改 `em`） | 单独决议 —— 见 §6 N6 |
...
| `textStyle` | 3 | 保留 | KRIG 自定义 |

**关于 bold→strong / italic→em 的争议（详 §6 N6）**：
- Markdown CommonMark 解析 `**text**` 为 `<strong>`，PM `prosemirror-schema-basic` 也用 `strong` / `em` 命名 —— 严格按阶梯 1，应改名。
- **但**：V1 现有代码 60+ 处使用 `bold` / `italic`，改名涉及全量重命名。
- **决议方向**：单独决议，需评估迁移成本 vs 标准对齐价值。本 RFC 暂保留 V1 命名。
...
| V1 | V2 提案 | 阶梯升级 | 决议状态 |
|---|---|---|---|
| `paragraph.children` | `paragraph.content` | 2（PM 标准命名） | 提案 |
| `heading.children` | `heading.content` | 2（PM 标准命名） | 提案 |
| `heading.level: 1-3` | `heading.level: 1-6` | 1（CommonMark 标准范围） | 提案 |
| `codeBlock.language` | `codeBlock.info` | 1（Markdown 标准命名） | 提案 |
| `image.caption` 当 alt 用 | 新增 `image.alt` + 保留 `caption` | 1（Markdown 标准） + 2（HTML5 figcaption） | 提案 |
| `taskItem.deadline` | `taskItem.due` | 3（GTD 通用） | 提案 |
| `table.colCount` | `table.columns` | 3 | 提案 |
| `list.listType` | （删除字段，通过节点 type 区分） | — | 提案 |
| `bold` mark | （提案 `strong`） | 1（Markdown / PM 标准） | **待决议**（§6 N6） |
| `italic` mark | （提案 `em`） | 1（Markdown / PM 标准） | **待决议**（§6 N6） |
...
| N3 | `level` 范围 1-6 扩展后，UI 是否真支持 H4-H6 渲染？影响 V2 NoteView capability 范围 | **schema 支持 1-6**，capability.text-editing 可选择性渲染 | Phase 2c |
| N4 | mathBlock 的 `color` / `bgColor` —— 真删除还是保留为 KRIG 扩展？V1 math 渲染依赖它们 | **临时保留**（标 KRIG 扩展 attrs），Phase 2b 看是否真有人用 | Phase 2b |
| N5 | `originalSrc` 删除后，V1 image 历史数据怎么处理（外部 URL 引用丢了） | **走边表达**（`*:prov:wasInformedBy`），但 V2 无历史数据要迁移 | 不阻塞 |
| **N6** | **Mark 命名是否对齐 PM/Markdown 标准（`bold`↔`strong`, `italic`↔`em`）？** | **临时保留 V1 `bold` / `italic`**。改名涉及 V2 现有代码 60+ 处。 | **单独决议** —— 评估迁移成本 vs 标准对齐价值 |
| N7 | V1 `image.caption` 同时承载 alt 和 caption 语义。迁移时如何拆分到 V2 的 alt 字段 vs caption 字段？ | **本 RFC 不提供自动迁移**，逐 atom 人工判断 | Phase 2b（结合真实数据决议） |
| N8 | 当 HTML 与 PM 命名冲突时（如 `start` vs `order`），本 RFC 规则"PM 优先"是否覆盖所有情况？还是有例外？ | **PM 优先作为默认规则**，例外单独决议 | Phase 2b 视具体冲突决议 |
