...
V2 区分两层正交的"隔离粒度",对应不同用户场景。

### 6.1 Workspace 级隔离(高级:完全隔离)

**用户场景**:多用户身份 / 不同环境 / 强隔离工作上下文。

**实现机制**:
- 每 Workspace 独立 partition(electron `session.fromPartition('persist:ws-<id>')`)
- 不同 Workspace 间:cookie / 登录态 / 历史 / 缓存 / 全部独立
- Workspace 实例完全自包含(NavSide / Slot / Overlay 各自独立)

**典型场景**:
- WS-A 用工作账号登录 ChatGPT,WS-B 用个人账号登录 ChatGPT
- WS-A 测试环境(localhost),WS-B 生产环境
- WS-A 学习上下文,WS-B 工作上下文

### 6.2 View 内部 Tab(浏览常态:轻隔离)

**用户场景**:浏览器多 tab 等 web 浏览习惯 — 一个上下文内的多页面任务。

**实现机制**:
- 同一 view 实例内多 tab(各 tab 独立 webContents / URL / 历史栈)
- tab 之间:**共享 partition**(同 Workspace) → cookie / 登录态共享
- tab 管理 UI(tab bar / 新建 / 关闭 / 切换)由对应 capability 内部提供
- **不抽象成通用 capability.tab-management**——这是特定 view 的浏览特性,非通用机制

**典型场景**:
- WebView 内开 chatgpt + github + 文档 多个 tab(共享 cookie / 登录态)
- 用户从 ChatGPT 弹出新链接 → 新 tab 打开,与原 tab 共存

**实施归属**(Q3=B 决策):
- Web tab 管理 → `capability.web-rendering` 内部职责(web 特有的浏览能力)
- view(`web-browser`)通过 install 自动获得
- 不为 NoteView / GraphView 等抽象通用 tab 机制(view 平等 + 复杂度控制)

### 6.3 两层正交的工作流

```
┌─ Workspace A(partition: persist:ws-a)─────────────────────┐
│  ├─ NoteView Slot                                           │
│  └─ WebView Slot                                            │
│     ├─ Tab 1: chatgpt.com (cookie: 工作账号 cookie)          │
│     ├─ Tab 2: github.com  ← 共享 ws-a 的 cookie             │
│     └─ Tab 3: ...                                            │
└──────────────────────────────────────────────────────────────┘

┌─ Workspace B(partition: persist:ws-b,完全独立)──────────┐
│  └─ WebView Slot                                            │
│     ├─ Tab 1: chatgpt.com (cookie: 个人账号 cookie,与 ws-a 隔离)│
│     └─ ...                                                   │
└──────────────────────────────────────────────────────────────┘
```
...
V1:**只有 Workspace 级"切显示"**(WebView 内不支持多 tab),用户失去 Chrome 浏览习惯。

V2:**两层正交**,既支持 Chrome 风格多 tab(浏览常态),又支持 Workspace 完全隔离(高级场景)。
...
| 2026-05-05 | v1.1 | 加 § 6 "隔离粒度的两层正交":Workspace 级(完全隔离,不同 partition)+ View 内 Tab(浏览常态,共享 partition);明确 web tab 归 capability.web-rendering 内部(Q3=B 决策),不抽象通用 tab-management |
